...
impl PreparedTriangle {
    fn indices(self) -> [usize; 3] {
        self.indices.map(|index| index as usize)
    }

    fn normal_index(self) -> Option<usize> {
        unpack_index(self.normal_index)
    }

    fn uvs(self) -> Option<[Vec2; 3]> {
        self.has_uvs.then_some(self.uvs)
    }
}
...
impl<'a> PreparedMesh<'a> {
    ...
    pub fn new(mesh: &'a Mesh) -> Self {
        let prepared = PreparedData::new(mesh);
        Self::from_data(mesh, prepared)
    }

    /// Build compact rendering data and release the source face graph.
    ///
    /// Animation, geometry, materials, and textures remain available through the
    /// prepared mesh. Only the allocation-heavy source polygons are discarded
    /// after their render-time topology has been flattened.
    #[must_use]
    pub fn new_compact(mesh: &'a mut Mesh) -> Self {
        let prepared = PreparedData::new(mesh);
        mesh.faces.clear();
    ...
    fn from_data(mesh: &'a Mesh, prepared: PreparedData) -> Self {
        Self {
            mesh,
            ...
            deferred_opaque: prepared.deferred_opaque,
        }
    }

    fn material(&self, face: &PreparedFace) -> Option<&Material> {
        unpack_index(face.material_index).and_then(|index| self.mesh.materials.get(index))
    }

    fn texture(&self, index: u32) -> Option<&Texture> {
        unpack_index(index).and_then(|index| self.mesh.textures.get(index))
    }

    fn emissive_map(&self, face: &PreparedFace) -> Option<&[[u8; 3]]> {
        unpack_index(face.material_index)
            .and_then(|index| self.emissive_maps.get(index))
            .and_then(|map| map.as_deref())
    }
}
...
impl PreparedData {
    fn new(mesh: &Mesh) -> Self {
        ...
        for face in &mesh.faces {
            ...
            let diffuse_texture_index = material
            ...
            for corners in triangulate_corners(face.indices.len()) {
                ...
                let uvs = triangle_uvs(mesh, face, corners);
                triangles.push(PreparedTriangle {
                    indices: [pack_index(a), pack_index(b), pack_index(c)],
                    normal_index: pack_optional_index(normal_index),
                    uvs: uvs.unwrap_or_default(),
                    has_uvs: uvs.is_some(),
                });
            }
            faces.push(PreparedFace {
                triangles: start..pack_index(triangles.len()),
                material_index: pack_optional_index(material_index),
                diffuse_texture_index: pack_optional_index(diffuse_texture_index),
                emissive_texture_index: pack_optional_index(emissive_texture_index),
                alpha_mode,
...
fn pack_index(index: usize) -> u32 {
    u32::try_from(index).expect("prepared mesh index exceeds u32")
}

fn pack_optional_index(index: Option<usize>) -> u32 {
    index.map_or(MISSING_INDEX, pack_index)
}
...
fn render_mesh_impl<M: Metrics>(
    ...
    let fill_projected = |projected: &mut Vec<ProjectedVertex>| {
        projected.clear();
        projected.extend(vertices.iter().map(|&v| {
            ...
            if config.flip_horizontal {
                projected.x = f32::from(area.width.saturating_sub(1)) - projected.x;
            }
            projected
        }));
    };

    let mut frame_scratch = prepared.map(|prepared| {
        prepared
            .frame_scratch
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
    ...
    let mut local_zbuf = Vec::new();
    let phase_started = metrics.start();
    if let Some(scratch) = frame_scratch.as_deref_mut() {
        fill_projected(&mut scratch.projected);
    } else {
        fill_projected(&mut local_projected);
    }
    ...
    let (projected, zbuf) = if let Some(scratch) = frame_scratch.as_deref_mut() {
        (&scratch.projected[..], &mut scratch.zbuf[..])
    } else {
        (&local_projected[..], &mut local_zbuf[..])
    };
    ...
    if let Some(prepared) = prepared {
        if prepared.can_defer_opaque(config) {
            draw_prepared_deferred(prepared, &ctx, area, buf, zbuf, config, metrics);
        } else {
            draw_prepared_faces(prepared, &ctx, area, buf, zbuf, config, metrics);
        }
        ...
        for face_index in 0..limit {
            ...
            draw_face(&ctx, face_index, material, area, buf, zbuf, config, metrics);
        }
...
fn draw_prepared_deferred<M: Metrics>(
    ...
    scratch.shaders.clear();
    let shader_capacity = prepared.triangles.len().min(32_768);
    let shader_reserve = shader_capacity.saturating_sub(scratch.shaders.capacity());
    scratch.shaders.reserve(shader_reserve);
    let DeferredScratch {
        occupied,
        cells,
        shaders,
    ...
    for face_index in 0..limit {
        ...
        for triangle_index in face.triangles.clone() {
            metrics.triangle();
            let triangle = prepared.triangles[triangle_index as usize];
            ...
            let shading = prepared_face_shading(
                prepared,
                face,
                triangle,
                config,
                ctx.glyph_ramp_is_ascii,
                intensity,
                '#',
            );
            let shader = DeferredShader {
                face_index: pack_index(face_index),
                triangle_index,
                intensity,
    ...
    for (index, deferred) in cells.iter().enumerate() {
        ...
        let face = &prepared.faces[shader.face_index as usize];
        let triangle = prepared.triangles[shader.triangle_index as usize];
        let shading = prepared_face_shading(
            prepared,
            face,
            triangle,
        ...
        let fragment = if shader.fast {
            shade_opaque_texture_cell(&shading, deferred.weights, config)
        } else {
            shade_cell(&shading, deferred.weights, config)
        };
...
fn draw_prepared_faces<M: Metrics>(
    ...
    if prepared.has_blend {
        ...
        blend.sort_by(|&a, &b| {
            prepared_face_depth(prepared, ctx.projected, b)
                .partial_cmp(&prepared_face_depth(prepared, ctx.projected, a))
...
fn prepared_face_depth(
    ...
    let mut sum = 0.0;
    let mut count = 0.0;
    for triangle_index in prepared.faces[face_index].triangles.clone() {
        for index in prepared.triangles[triangle_index as usize].indices() {
            if let Some(vertex) = projected.get(index) {
                sum += vertex.depth;
                count += 1.0;
            }
        }
...
fn prepared_face_shading<'a>(
    prepared: &'a PreparedMesh<'_>,
    face: &PreparedFace,
    ...
    let indices = triangle.indices();
    let material = prepared.material(face);
    FaceShading {
        material,
        uvs: triangle.uvs(),
        vertex_colors: triangle_vertex_colors_for_indices(prepared.mesh, indices),
        diffuse_texture: texture_enabled(config.color_mode)
            .then(|| prepared.texture(face.diffuse_texture_index))
            .flatten(),
        emissive_texture: texture_enabled(config.color_mode)
            .then(|| prepared.texture(face.emissive_texture_index))
            .flatten(),
        emissive_map: prepared.emissive_map(face),
...
fn draw_prepared_face<M: Metrics>(
    ...
    for triangle_index in face.triangles.clone() {
        ...
        if matches!(config.render_mode, RenderMode::Solid) {
            ...
            let style = style_for(prepared.material(face), None, intensity, config);
...
mod tests {
    ...
    #[test]
    fn compact_prepared_render_releases_source_faces() {
        let mut mesh = quad_mesh();
        let config = Mesh3dConfig::default().backface_culling(false);
        let direct = render(&mesh, &config);
        let prepared = PreparedMesh::new_compact(&mut mesh);
        assert!(prepared.mesh().faces.is_empty());
        ...
        assert_eq!(
            direct.backend().buffer().content(),
            compact.backend().buffer().content()
        );
    }
