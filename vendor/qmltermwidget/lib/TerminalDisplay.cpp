...
constexpr int GlyphFirst = 32;
constexpr int GlyphCount = 96;
constexpr int GlyphColumns = 16;
constexpr int GlyphRows = GlyphCount / GlyphColumns;
...
const QSGGeometry::AttributeSet &terminalAttributes()
{
...
class TerminalMaterialShader final : public QSGMaterialShader
{
public:
    TerminalMaterialShader()
    {
        setShaderFileName(
            VertexStage,
            QStringLiteral(":/qmltermwidget/shaders/shaders/terminal.vert.qsb"));
        setShaderFileName(
            FragmentStage,
            QStringLiteral(":/qmltermwidget/shaders/shaders/terminal.frag.qsb"));
    }
        ...
        QSGMaterial *) override
    {
        ...
        if (state.isOpacityDirty()) {
            ...
            std::memcpy(uniforms->data() + 64, &opacity, 4);
            changed = true;
        }
        return changed;
    }
...
class TerminalMaterial final : public QSGMaterial
{
public:
    TerminalMaterial()
    {
    ...
    {
        const auto *otherMaterial = static_cast<const TerminalMaterial *>(other);
        if (_texture == otherMaterial->_texture) {
            return 0;
        }
        return _texture < otherMaterial->_texture ? -1 : 1;
    }
...
void TerminalMaterialShader::updateSampledImage(
    RenderState &state,
    int binding,
    QSGTexture **texture,
    QSGMaterial *newMaterial,
    QSGMaterial *)
{
    if (binding == 1) {
        auto *atlas =
            static_cast<TerminalMaterial *>(newMaterial)->texture();
        if (!atlas) {
            *texture = nullptr;
            return;
        }
        // Custom materials must upload newly-created QSGPlainTextures before
        // the RHI samples them. Without this, the geometry is valid but the
        // atlas remains transparent on Plasma's threaded render loop.
        atlas->commitTextureOperations(
            state.rhi(),
...
class TerminalGlyphNode final : public QSGGeometryNode
{
public:
    TerminalGlyphNode()
        : _geometry(new QSGGeometry(terminalAttributes(), 0))
        , _material(new TerminalMaterial)
    {
        _geometry->setDrawingMode(QSGGeometry::DrawTriangles);
        _geometry->setVertexDataPattern(QSGGeometry::DynamicPattern);
    ...
    void updateAtlas(
        ...
        int lineSpacing)
    {
        if (_texture && _font == font && _cellSize == cellSize
            && _ascent == ascent && _lineSpacing == lineSpacing) {
            return;
        }
        ...
        _tileSize = QSize(
            cellSize.width() + 2 * GlyphPadding,
            cellSize.height() + 2 * GlyphPadding);
        _atlasSize = QSize(
            _tileSize.width() * GlyphColumns,
            _tileSize.height() * GlyphRows);

        QImage atlas(_atlasSize, QImage::Format_RGBA8888_Premultiplied);
        ...
        for (int index = 0; index < GlyphCount; ++index) {
            const int column = index % GlyphColumns;
            const int row = index / GlyphColumns;
            const char32_t codepoint = GlyphFirst + index;
            painter.drawText(
                column * _tileSize.width() + GlyphPadding,
                row * _tileSize.height() + GlyphPadding + ascent + lineSpacing,
        ...
        _texture->setFiltering(QSGTexture::Linear);
        _texture->setHorizontalWrapMode(QSGTexture::ClampToEdge);
        _texture->setVerticalWrapMode(QSGTexture::ClampToEdge);
        _material->setTexture(_texture);
        markDirty(QSGNode::DirtyMaterial);
    }

    TerminalVertex *beginGeometry(int maximumGlyphs)
    {
        const int requiredVertices = maximumGlyphs * 6;
        if (requiredVertices > _vertexCapacity) {
            _geometry->allocate(requiredVertices);
            _vertexCapacity = requiredVertices;
        }
        return static_cast<TerminalVertex *>(_geometry->vertexData());
    }

    void finishGeometry(int glyphCount)
    {
#if QT_VERSION >= QT_VERSION_CHECK(6, 10, 0)
        _geometry->setVertexCount(glyphCount * 6);
#else
        const int usedVertices = glyphCount * 6;
        const int unusedVertices = _geometry->vertexCount() - usedVertices;
        if (unusedVertices > 0) {
            auto *vertices = static_cast<TerminalVertex *>(_geometry->vertexData());
            std::memset(
                vertices + usedVertices,
                0,
                static_cast<size_t>(unusedVertices) * sizeof(TerminalVertex));
        }
#endif
        markDirty(QSGNode::DirtyGeometry);
    }

    QRectF textureRect(char32_t codepoint) const
    {
        int index = static_cast<int>(codepoint) - GlyphFirst;
        ...
        const int row = index / GlyphColumns;
        return QRectF(
            static_cast<qreal>(column * _tileSize.width()) / _atlasSize.width(),
            static_cast<qreal>(row * _tileSize.height()) / _atlasSize.height(),
            static_cast<qreal>(_tileSize.width()) / _atlasSize.width(),
            static_cast<qreal>(_tileSize.height()) / _atlasSize.height());
    }
...
,_image(nullptr)
,_imageSize(0)
,_sharedFrameMode(false)
,_sharedFrameTimer(nullptr)
...
,_mouseAutohideDelay(-1)
{
  ...
  _sharedFrameTimer = new QTimer(this);
  _sharedFrameTimer->setTimerType(Qt::PreciseTimer);
  _sharedFrameTimer->setInterval(16);
  connect(
      _sharedFrameTimer,
      &QTimer::timeout,
      this,
      &TerminalDisplay::pollSharedFrame);
...
void TerminalDisplay::setSharedFrameMode(bool enabled)
{
  if (_sharedFrameMode == enabled)
    return;

  _sharedFrameMode = enabled;
  if (enabled)
  {
    _sharedFrameTimer->start();
    pollSharedFrame();
  }
  else
  {
...
{
  if (!_sharedFramePath.isEmpty())
    return _sharedFramePath;

  return QStandardPaths::writableLocation(QStandardPaths::RuntimeLocation)
      + QStringLiteral("/desktop-tui-widget-frame.bin");
}

void TerminalDisplay::setSharedFramePath(const QString &path)
{
  if (_sharedFramePath == path)
    return;

  const bool restart = _sharedFrameMode;
  if (restart)
    setSharedFrameMode(false);
  _sharedFramePath = path;
  emit sharedFramePathChanged();
  if (restart)
    setSharedFrameMode(true);
}

bool TerminalDisplay::mouseInputEnabled() const
{
  return acceptedMouseButtons() != Qt::NoButton;
}

void TerminalDisplay::setMouseInputEnabled(bool enabled)
{
...
void TerminalDisplay::pollSharedFrame()
{
  if (!_sharedFrameMode)
    return;
  ...
  if (!_sharedFrameMap)
  {
    const QString path = sharedFramePath();
    ...
    if (std::memcmp(mapping, SharedFrameMagic, sizeof(SharedFrameMagic)) != 0)
    {
      file->unmap(mapping);
      delete file;
      return;
    ...
    const quint32 columns = qFromLittleEndian<quint32>(mapping + 8);
    const quint32 lines = qFromLittleEndian<quint32>(mapping + 12);
    const quint32 stride = qFromLittleEndian<quint32>(mapping + 24);
    const qint64 cellBytes =
        static_cast<qint64>(columns) * lines * SharedFrameCellSize;
    const qint64 expectedSize =
        SharedFrameHeaderSize + 2 * cellBytes;
    if (columns == 0 || lines == 0 || columns > 4096 || lines > 4096
        || stride != SharedFrameCellSize || expectedSize != size)
    {
      file->unmap(mapping);
      delete file;
      return;
    ...
    _sharedFrameFile = file;
    _sharedFrameMap = mapping;
  ...
  const quint64 state =
      qFromLittleEndian<quint64>(_sharedFrameMap + 16);
  if (state == 0 || state == _sharedFrameState)
    return;

  const qsizetype cellBytes = _sharedFrameCells.size();
  const qsizetype bufferIndex = static_cast<qsizetype>(state & 1);
  const uchar *source =
      _sharedFrameMap + SharedFrameHeaderSize + bufferIndex * cellBytes;
  std::atomic_thread_fence(std::memory_order_acquire);
  std::memcpy(_sharedFrameCells.data(), source, cellBytes);
  std::atomic_thread_fence(std::memory_order_acquire);
  if (qFromLittleEndian<quint64>(_sharedFrameMap + 16) != state)
    return;

  _sharedFrameState = state;
  update();
}
...
//  _screenWindow->resetScrollCount();
  ...
  for (y = 0; y < linesToUpdate; ++y)
  {
    ...
    int firstDirtyColumn = -1;
    int lastDirtyColumn = -1;
    int previousDirtyColumn = -1;
    ...
    for( x = 0 ; x < columnsToUpdate ; ++x)
    {
      ...
      if (newLine[x] != currentLine[x]) {
        if (firstDirtyColumn < 0) {
          firstDirtyColumn = x;
        }
        if (previousDirtyColumn >= 0 &&
            x - previousDirtyColumn > largestGapEnd - largestGapStart) {
          largestGapStart = previousDirtyColumn + 1;
          largestGapEnd = x - 1;
        }
        previousDirtyColumn = x;
        lastDirtyColumn = x;
      }
    }
    ...
    if (_lineProperties.count() > y) {
        if ((_lineProperties[y] & LINE_DOUBLEHEIGHT) != 0) {
            firstDirtyColumn = 0;
            lastDirtyColumn = columnsToUpdate - 1;
            largestGapStart = -1;
            largestGapEnd = -1;
        }
    }
    ...
    if (firstDirtyColumn >= 0)
    {
        // Pad by one cell for glyphs that overhang their nominal fixed-width cell.
        auto addDirtyColumns = [&](int firstColumn, int lastColumn) {
            firstColumn = qMax(0, firstColumn - 1);
            lastColumn = qMin(columnsToUpdate - 1, lastColumn + 1);
            if (firstColumn <= lastColumn) {
                dirtyRegion |= QRect(
                    _leftMargin + tLx + _fontWidth * firstColumn,
                    _topMargin + tLy + _fontHeight * y,
                    _fontWidth * (lastColumn - firstColumn + 1),
                    _fontHeight);
            }
        };

        // Animated terminal art often has two moving regions separated by a large,
        // unchanged center. Preserve that clean gap instead of repainting the whole row.
        if (largestGapEnd - largestGapStart + 1 >= 8) {
            addDirtyColumns(firstDirtyColumn, largestGapStart - 1);
            addDirtyColumns(largestGapEnd + 1, lastDirtyColumn);
        } else {
            addDirtyColumns(firstDirtyColumn, lastDirtyColumn);
        }
    }
...
void TerminalDisplay::paint(QPainter *painter)
{
    ...
    if (_opacity < static_cast<qreal>(1))
    {
        painter->save();
        painter->setCompositionMode(QPainter::CompositionMode_Source);
        painter->fillRect(cr, Qt::transparent);
        painter->restore();
    }
...
QSGNode *TerminalDisplay::updatePaintNode(QSGNode *oldNode, UpdatePaintNodeData *data)
{
    ...
    const bool hasSharedFrame =
        _sharedFrameMode
        && _sharedFrameColumns > 0
        && _sharedFrameLines > 0
        && _sharedFrameCells.size()
            == _sharedFrameColumns * _sharedFrameLines * SharedFrameCellSize;
    if (!window()
        || (_sharedFrameMode && !hasSharedFrame)
        || (!hasSharedFrame
            && (!_image || _usedLines <= 0 || _usedColumns <= 0))) {
        delete oldNode;
        return nullptr;
    }

    auto *glyphNode = static_cast<TerminalGlyphNode *>(oldNode);
    if (!glyphNode) {
        glyphNode = new TerminalGlyphNode;
    }
    glyphNode->updateAtlas(
        window(),
        m_font,
        QSize(_fontWidth, _fontHeight),
        _fontAscent,
        _lineSpacing);

    const int lines = hasSharedFrame ? _sharedFrameLines : _usedLines;
    const int columns = hasSharedFrame ? _sharedFrameColumns : _usedColumns;
    TerminalVertex *vertices = glyphNode->beginGeometry(lines * columns);
    int glyphCount = 0;
    const auto appendGlyph =
        [&](int x, int y, char32_t codepoint, const QColor &foreground) {
        const QRectF texture = glyphNode->textureRect(codepoint);
        const QSize tile = glyphNode->tileSize();
        const float left = _leftMargin + x * _fontWidth - GlyphPadding;
        const float top = _topMargin + y * _fontHeight - GlyphPadding;
        const float right = left + tile.width();
        const float bottom = top + tile.height();
        const float textureLeft = texture.left();
        const float textureTop = texture.top();
        const float textureRight = texture.right();
        const float textureBottom = texture.bottom();

        TerminalVertex *glyph = vertices + glyphCount * 6;
        setTerminalVertex(
            glyph[0], left, top, textureLeft, textureTop, foreground);
        setTerminalVertex(
            glyph[1], right, top, textureRight, textureTop, foreground);
        setTerminalVertex(
            glyph[2], left, bottom, textureLeft, textureBottom, foreground);
        setTerminalVertex(
            glyph[3], left, bottom, textureLeft, textureBottom, foreground);
        setTerminalVertex(
            glyph[4], right, top, textureRight, textureTop, foreground);
        setTerminalVertex(
            glyph[5], right, bottom, textureRight, textureBottom, foreground);
        ++glyphCount;
    };

    if (hasSharedFrame) {
        const auto *cells =
            reinterpret_cast<const uchar *>(_sharedFrameCells.constData());
        for (int y = 0; y < lines; ++y) {
            for (int x = 0; x < columns; ++x) {
                const uchar *cell =
                    cells
                    + (y * columns + x) * SharedFrameCellSize;
                const char32_t codepoint =
                    static_cast<char32_t>(
                        qFromLittleEndian<quint32>(cell));
                ...
                appendGlyph(
                    ...
                    y,
                    codepoint,
                    QColor(cell[4], cell[5], cell[6], cell[7]));
            }
        }
    } else {
        for (int y = 0; y < lines; ++y) {
            for (int x = 0; x < columns; ++x) {
                const Character &cell = _image[loc(x, y)];
                if (cell.character == 0 || cell.character == ' '
                    || (cell.rendition & RE_CONCEAL)
                    || (_blinking && (cell.rendition & RE_BLINK))) {
                    continue;
                }

                char32_t codepoint =
                    static_cast<char32_t>(cell.character);
                if (cell.rendition & RE_EXTENDED_CHAR)
                    codepoint = '?';

                QColor foreground =
                    cell.foregroundColor.color(_colorTable);
                if ((cell.rendition & RE_CURSOR) && !_cursorBlinking
                    && !_cursorColor.isValid()) {
                    foreground =
                        cell.backgroundColor.color(_colorTable);
                }
                appendGlyph(x, y, codepoint, foreground);
            }
        }
    }
    glyphNode->finishGeometry(glyphCount);
    return glyphNode;
}
